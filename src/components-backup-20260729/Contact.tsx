...
// Web3Forms access key. Set NEXT_PUBLIC_WEB3FORMS_KEY in Vercel → Project → Environment
// Variables. It is a public submission key by design (it only lets a form post TO an inbox),
// so NEXT_PUBLIC_ is correct here — but it must be a real key.
...
export default function Contact() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // No key configured = the message has nowhere to go. Say so instead of pretending.
    // This component previously shipped with the literal placeholder "YOUR_ACCESS_KEY_HERE"
    // AND swallowed every error into a success screen, so a live prospect saw
    // "Inquiry received, we'll contact you within 48 hours" while the message went nowhere.
    if (!WEB3FORMS_KEY) {
      setFailed(true);
      setLoading(false);
      return;
        ...
        >
          {submitted ? (
            ...
            <motion.div initial={false} animate={{ opacity: 1 }} className="text-center py-12">
              ...
              <a href={`mailto:${MAILTO}?subject=Assessment%20Inquiry`} className="btn-primary inline-flex py-3 px-8 text-sm">
                {MAILTO}
              </a>
            </motion.div>
          ) : (
